...
const accentClasses = {
  amber: { text: 'text-amber', dot: 'bg-amber', tag: 'border-amber/30 text-amber' },
  mint: { text: 'text-mint', dot: 'bg-mint', tag: 'border-mint/30 text-mint' },
};
  ...
  return (
    <div className="flex h-full flex-col bg-gradient-to-b from-base-850 to-base-900 px-4 pt-10">
        ...
        {[
          ...
          <div key={s.time} className="glass-strong flex items-center gap-3 rounded-xl p-3">
            ...
            <div className="min-w-0">
              <p className="truncate text-xs text-ink-100">{s.title}</p>
              <p className="text-[10px] text-ink-500">{s.time}</p>
            </div>
          </div>
        ))}
      </div>
      <div className="glass-strong mt-4 flex items-center justify-center gap-2 rounded-xl p-3">
        ...
        <span className="text-xs text-ink-100">Check in</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex h-full flex-col bg-gradient-to-b from-base-850 to-base-900 px-4 pt-10">
      ...
      <div className="glass-strong mt-5 rounded-xl p-4">
        ...
        <p className="mt-1 font-display text-xl font-semibold">₹24,680</p>
      ...
      <div className="glass-strong mt-4 rounded-xl p-3">
        <p className="font-mono text-[9px] text-mint">Say it, don't type it</p>
        <p className="mt-2 text-xs text-ink-100">"Groceries, 1200 rupees"</p>
      </div>
    </div>
  );
}
...
export default function ProjectCard({ project, reverse = false }) {
  ...
  return (
    <motion.div
      ...
      className={`glass grid grid-cols-1 items-center gap-10 rounded-tile p-8 sm:p-10 lg:grid-cols-[0.9fr_1.1fr] ${reverse ? 'lg:[&>*:first-child]:order-2' : ''
        }`}
    >
      ...
      <div>
        <div className="mb-2 flex items-center gap-2">
          ...
          <span className={`font-mono text-xs ${accent.text}`}>{project.role}</span>
        </div>
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          {project.stack.map((t) => (
            <span
              key={t}
              className={`rounded-full border px-3 py-1 text-xs ${accent.tag}`}
            >
              {t}
            </span>
          ))}
